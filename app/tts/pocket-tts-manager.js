(() => {
  ...
  class PocketTTSManager extends EventTarget {
    ...
    speak(agent,text,voiceId){ const s=settings(); if(!s.enabled||!text?.trim())return; const cfg=window.geebrWorld?.getBrainConfig?.(agent.id)||{}; if(cfg.ttsEnabled===false)return; const item={agent,text:String(text).slice(0,500),voice:voiceId||cfg.ttsVoiceId||'builtin:alba',at:Date.now(),speakAt:performance.now()};
      const last=this.queue[this.queue.length-1];
      if(last&&last.agent===agent){ last.text=(last.text+' '+item.text).slice(0,500); last.at=item.at; }
      else this.queue.push(item); if(this.queue.length>8)this.queue.splice(0,this.queue.length-8); this.pump(); }
