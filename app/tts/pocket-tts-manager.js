(() => {
  ...
  class PocketTTSManager extends EventTarget {
    ...
    async load(){ if(this.ready)return; if(this.loading)return this.pendingLoad; this.loading=true; this.status(this.wasLoaded()?'Auto-loading cached Pocket-TTS…':'Downloading Pocket-TTS model…','loading'); this.pendingLoad=new Promise((resolve,reject)=>{ this.worker=new Worker('./tts/inference-worker.js',{type:'module'}); this.worker.onmessage=e=>this.onMessage(e.data,resolve,reject); this.worker.onerror=e=>{this.loading=false;reject(e);this.status('Pocket-TTS failed: '+e.message,'error');}; this.worker.postMessage({type:'load',data:{language:settings().language}}); }); return this.pendingLoad; }
    async onMessage(m,resolve,reject){ if(m.type==='runtime_info')Object.assign(this.telemetry,m.data||{}); if(m.type==='bundle_loaded')this.sampleRate=m.sampleRate||24000; if(m.type==='voices_loaded'||m.type==='loaded'){ if(!this.ready){this.ready=true;this.loading=false;localStorage.setItem(CACHE_MARKER,'1');this.status('Pocket-TTS ready (WASM CPU)','ready');resolve?.(); await this.restoreCustomVoices();} }
      ...
      if(m.type==='stream_ended'){if(this.directPort){if(this.receivedSamples)this.finishCurrent(true);else this.finishCurrent(false);}else if(this.receivedSamples){this.pendingStreamEnd=true;this.flushAudio();}else this.finishCurrent(false);}
      if(m.type==='voice_encoded'){ const p=this.pending.get('encode'); if(p){this.pending.delete('encode');p.resolve();} }
      if(m.type==='error'){this.status(m.error,'error'); const p=this.pending.get('encode');if(p){this.pending.delete('encode');p.reject(new Error(m.error));} if(this.running)this.finishCurrent(false); reject?.(new Error(m.error));} }
    speak(agent,text,voiceId){ const s=settings(); if(!s.enabled||!text?.trim())return; const cfg=window.geebrWorld?.getBrainConfig?.(agent.id)||{}; if(cfg.ttsEnabled===false)return; this.queue.push({agent,text:String(text).slice(0,500),voice:voiceId||cfg.ttsVoiceId||'builtin:alba',at:Date.now(),speakAt:performance.now()}); if(this.queue.length>8)this.queue.splice(0,this.queue.length-8); this.pump(); }
    async pump(){ if(this.running||!this.queue.length)return; this.running=true; try{await this.load();await this.ensureAudio(); if(this.audioCtx.state!=='running')throw new Error('Audio is blocked; click the page once, then try again.'); let item; do{item=this.queue.shift();}while(item&&Date.now()-item.at>20000); if(!item)return this.finishCurrent(false); this.current=item; this.dispatchEvent(new CustomEvent('speechstart',{detail:item})); const voice=item.voice.startsWith('custom:')?item.voice:item.voice.replace('builtin:',''); this.worker.postMessage({type:'generate',data:{text:item.text,voice}}); }catch(e){this.status(e.message,'error');this.finishCurrent(false);} }
  ...
  window.geebrTTS=new PocketTTSManager(); window.GEEBR_TTS_BUILTINS=BUILTINS;
})();
