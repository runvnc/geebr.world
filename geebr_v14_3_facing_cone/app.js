...
function buildVisiblePerception(agentId=null, radius=5){
  ...
  for(let z=cz-radius; z<=cz+radius; z++) for(let x=cx-radius; x<=cx+radius; x++){
    ...
    cells.set(gridKey(x,z), {base:visible?baseGlyphForTile(x,z):'?', marks:'', prio:visible?0:99, visible, inCone});
  }
  ...
  const legend=[
    'Legend: @ self, g other, , grass, : dirt/path, ~ water, ^ wall/stone, = bridge, C crate, B barrel, M mushroom, L lamp, * crystal/magic, H house, ? unseen/unknown/confused',
    'Vision model: forward-facing cone plus a 1-tile awareness bubble around the Geebr. Tiles behind the Geebr are mostly unknown unless adjacent.',
...
async function main(){ const engine=await createEngine(); state.engine=engine; const scene=new BABYLON.Scene(engine); state.scene=scene; scene.clearColor=new BABYLON.Color4(.055,.071,.088,1); const hk=await HavokPhysics(); scene.enablePhysics(new BABYLON.Vector3(0,-9.81,0),new BABYLON.HavokPlugin(true,hk));
  ...
  setupUI(); scene.onBeforeRenderObservable.add(()=>animate(engine.getDeltaTime()/1000)); engine.runRenderLoop(()=>scene.render()); window.addEventListener('resize',()=>engine.resize()); log('v14.3 loaded: facing-aware vision cone + adjacent awareness + LOS'); updatePerceptionUI(); }
