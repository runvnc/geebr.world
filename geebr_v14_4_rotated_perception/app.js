...
function buildVisiblePerception(agentId=null, radius=5){
  ...
  const f={x:Math.sign(g?.dir?.x||0), z:Math.sign(g?.dir?.z||-1) || -1};
  const r=rightVecFromDir(g.dir);
  const cellByWorld=new Map();
  const details=[];

  // Build a facing-relative perception map. Top of the text grid is always "in front".
  // fd = forward distance, sx = side offset (negative = left, positive = right).
  for(let fd=radius; fd>=-radius; fd--){
    for(let sx=-radius; sx<=radius; sx++){
      const x=cx + f.x*fd + r.x*sx;
      const z=cz + f.z*fd + r.z*sx;
      const inCone=isWithinFacingVision(g,cx,cz,x,z,radius);
      const visible=inCone && canSeeCell(cx,cz,x,z,opaque);
      const base=visible ? baseGlyphForTile(x,z) : ' ';
      cellByWorld.set(gridKey(x,z), {base, marks:'', prio:visible?0:99, visible, x, z});
    }
  }
  function put(x,z,base,marks,prio){
    const k=gridKey(x,z); const c=cellByWorld.get(k); if(!c || !c.visible) return;
    if(prio>=c.prio){ c.base=base; c.marks=marks||''; c.prio=prio; }
    else if(marks && !c.marks) c.marks=marks;
  }
  ...
  for(const other of state.geebrs){
    ...
    if(!isWithinFacingVision(g,cx,cz,x,z,radius)) continue;
    if(!canSeeCell(cx,cz,x,z,opaque)) continue;
    const isSelf=other===g;
    put(x,z,isSelf?'@':'g', other.anim==='panic'?'?':'', 90);
  ...
  for(let fd=radius; fd>=-radius; fd--){
    ...
    for(let sx=-radius; sx<=radius; sx++){
      const x=cx + f.x*fd + r.x*sx;
      const z=cz + f.z*fd + r.z*sx;
      const c=cellByWorld.get(gridKey(x,z)) || {base:' ', marks:''};
      row += compactCell(c.base,c.marks);
    }
    rows.push(`${String(fd).padStart(3,' ')} ${row}`);
  }
  ...
  const legend=[
    ...
    'Vision model: grid is rotated to match facing; top = in front, bottom = behind. Forward cone + 1-tile awareness bubble only.',
    'Hidden cells are blank spaces, not ? markers. Walls/stone/large solid objects still block line of sight behind them.',
  ...
  return [
    ...
    '',
    `Facing-relative ${(radius*2+1)}x${(radius*2+1)} map. Row labels are forward distance:`,
    ...rows,
    '',
    ...
    ...legend,
    '',
    'Suggested command JSON examples:',
    '{"kind":"look","targetId":"C1"}',
    '{"kind":"push","targetId":"B1"}',
    '{"kind":"spell","spell":"fireball","targetId":"C1"}'
  ].join('\n');
}
function updatePerceptionUI(){ const out=document.getElementById('perceptionOut'); if(!out) return; const r=document.getElementById('visionRadius')?.value || 5; out.textContent=buildVisiblePerception(null,r); }
...
async function main(){ const engine=await createEngine(); state.engine=engine; const scene=new BABYLON.Scene(engine); state.scene=scene; scene.clearColor=new BABYLON.Color4(.055,.071,.088,1); const hk=await HavokPhysics(); scene.enablePhysics(new BABYLON.Vector3(0,-9.81,0),new BABYLON.HavokPlugin(true,hk));
  ...
  setupUI(); scene.onBeforeRenderObservable.add(()=>animate(engine.getDeltaTime()/1000)); engine.runRenderLoop(()=>scene.render()); window.addEventListener('resize',()=>engine.resize()); log('v14.4 loaded: rotated facing-relative perception + blank hidden cells + LOS'); updatePerceptionUI(); }
